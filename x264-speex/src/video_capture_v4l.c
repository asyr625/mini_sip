...
int hdviper_capture_video_frame_v4l(Video *v) {
	...
	memcpy(v->yuvs[0], v4l2_data->mmap_buffers[buf.index].start, v4l2_data->mmap_buffers[buf.index].length);
	if (-1 == my_ioctl(v4l2_data->fd, VIDIOC_QBUF, &buf)) {
...
void v4l2_convert_to_BGRA(Video *v, int pixel_type) {
	...
	case V4L2_PIX_FMT_YUV420:
		yuv420p_to_bgra(v->width, v->height, v->rgb, v->yuvs[0]);
		break;
	case V4L2_PIX_FMT_YUYV:
		yuyv_to_bgra(v->width, v->height, v->rgb, v->yuvs[0]);
		break;
